...
class Vote extends REST_Controller
{
    function index_get()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this->response(array(
                    'error' => 'vote get request failure.'
                ), 404);
            } else {
    ...
    function index_put()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            $sanatizedPayload = $validData["payload"];
            $modelResult      = $this->vote_model->put($sanatizedPayload, $whereArray);
            if (!$modelResult) {
                $this->response(array(
                    'error' => 'vote put request failure.'
                ), 404);
            } else {
                $this->response($modelResult, 200);
            }
    ...
    function index_post()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this->response(array(
                    'error' => 'vote post request failure.'
                ), 404);
            } else {
                $this->response($modelResult, 200);
            }
    ...
    function index_delete()
    {
        ...
        if (!$validData["isValid"]) {
            ...
            if (!$modelResult) {
                $this->response(array(
                    'error' => 'vote delete request failure.'
                ), 404);
            } else {
                $this->response($modelResult, 200);
            }
